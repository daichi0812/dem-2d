...
struct Element
{
  ...
  void initialize( const int in_index, const ShapeTemplate2D* in_template_ptr, const double in_dx_sample_points )
  {
    index = in_index;
    template_ptr = in_template_ptr;
    clearForceAndTorque();

    mass = template_ptr->computeArea() * template_ptr->getDensity() * size_ratio * size_ratio;
    inertia = template_ptr->computeSecondMomentOfArea() * template_ptr->getDensity() * size_ratio * size_ratio * size_ratio * size_ratio;
    template_ptr->generateCollisionSamples( collision_samples, in_dx_sample_points );
