...
void RigidBody2DData::deserializeElements( const std::string& in_element_fn )
{
  HDF5File h5( in_element_fn, HDF5AccessType::READ_ONLY );
  m_Elements.clear();

  if( h5.doesGroupExist( "progress_data" ) )
  {
    h5.readScalar( "progress_data", "dt", m_ProgressData.dt );
    h5.readScalar( "progress_data", "time", m_ProgressData.time );
    h5.readScalar( "progress_data", "tick_count", m_ProgressData.tick_count );
    h5.readScalar( "progress_data", "serialization_idx", m_ProgressData.serialization_idx );
  }
  else
  {
  ...
  for( int i=0; ; i++ )
  {
    try
    {
      ...
      if( !h5.doesDatasetExist( "template_name_dict", dataset_name ) )
        break;
      std::string data_str;
      h5.readString( "template_name_dict", dataset_name, data_str );
  ...
  try
  {
    h5.readMatrix( "elements_2d", "angular_velocity", angular_velocity_array );
    h5.readMatrix( "elements_2d", "center_of_mass", center_of_mass_array );
    h5.readMatrix( "elements_2d", "rotation_angle", rotation_angle_array );
    h5.readMatrix( "elements_2d", "size_ratio", size_ratio_array );
    h5.readMatrix( "elements_2d", "static", isStatic_array );
    h5.readMatrix( "elements_2d", "template_idx", template_idx );
    h5.readMatrix( "elements_2d", "velocity", velocity_array );
  }
  catch( const std::string& e )
  {
    std::cout << "error: " << e << std::endl;
...
void RigidBody2DData::serializeElements( const std::string& in_element_fn, const ForceData2D& in_ForceData ) const
{
  HDF5File h5( in_element_fn, HDF5AccessType::READ_WRITE );
  _serializeElements( h5 );
  in_ForceData.serializeForces( h5 );
}

void RigidBody2DData::serializeForces( HDF5File& io_HDF5, const ForceData2D& in_ForceData, const std::string& in_time_step ) const
{
  _serializeElements( io_HDF5, in_time_step );
  in_ForceData.serializeForces( io_HDF5, in_time_step );
}
